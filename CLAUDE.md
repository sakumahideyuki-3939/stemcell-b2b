...
- [x] contact.phpのメールアドレス → info@algo-inc.com に差し替え済み
- [ ] GitHub Actions によるデプロイ自動化（現在は手動rsync）
- [x] OGP・SEO meta情報の追加（全ページ title/meta/OGP/canonical 設定済み）
- [ ] ブログ記事の実コンテンツ追加
- [ ] 画像の最適化（WebP変換・圧縮）
- [x] 全ページ共通CTA・スマホ固定CTAバー実装
- [x] index.php キャッチコピー刷新・信頼バッジ・導入クリニックの声（プレースホルダー）
- [x] concept.php 培養上清液 vs Lysate 比較表追加
- [x] evidence.php 学会発表セクション・成分分析データセクション追加
- [x] products.php 製品仕様表拡充（保存方法・使用期限・ロットサイズ）
- [x] about.php 「なぜ生搾りか」ストーリーセクション追加
- [x] company.php 製造品質管理セクション追加
- [x] footer.php 医療従事者向け注意書き・法律遵守バッジ追加
- [x] contact.php 問い合わせ種別を更新（資料請求・サンプル依頼・導入相談・価格確認・その他）
- [x] sitemap.xml about.php追加
- [x] 薬機法・医療広告ガイドライン準拠チェック完了
...
- **日付**: 2026-03-07
- **更新者**: Hide
- **内容**: サイト理想系完全実装（CTA・SEO・エビデンス充実・薬機法対応）
- **次回やること**: 導入クリニックの声を実データに差し替え、学会発表セクションの実データ入力、ブログ記事追加、GA4タグ設置、デプロイ自動化検討
